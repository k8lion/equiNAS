...
class MixedLiftingConv2d(torch.nn.Module):
    def __init__(self, group: tuple, in_channels: int, out_channels: int, kernel_size: int, padding: int = 0, bias: bool = True, baseline: bool = False, prior: bool = True, discrete: bool = False, norm: bool = True, skip: bool = False, alphas: torch.Tensor = None):
    ...
    def distance(self, supergroup):
        ...
        copied_filters = copied_filters.reshape(int(self.out_channels//groupsize(supergroup)), groupsize(supergroup), 
                                                self.in_channels, self.kernel_size, self.kernel_size)
        if supergroup != (1,2):
            raise NotImplementedError
        inv_order = [D4inverse((j,i)) for j in range(subgroupsize(supergroup, 0)) for i in range(subgroupsize(supergroup, 1))]
        for i in range(groupsize(supergroup)):
            copied_filters[:,i] = rotateflip_n(copied_filters[:,i], inv_order[i][1], subgroupsize(supergroup, 1), inv_order[i][0], subgroupsize(supergroup, 0))
        projected_weights = copied_filters.mean(dim=1)
...
class MixedGroupConv2d(torch.nn.Module):
    def __init__(self, group: tuple, in_channels: int, out_channels: int, kernel_size: int, padding: int = 0, bias: bool = True, baseline: bool = False, prior: bool = True, discrete: bool = False, norm: bool = True, skip: bool = True, alphas: torch.Tensor = None):
    ...
    def distance(self, supergroup):
        ...
        copied_filters = copied_filters.reshape(int(self.out_channels//groupsize(supergroup)), groupsize(supergroup), 
                                                int(self.in_channels//groupsize(supergroup)), groupsize(supergroup), 
                                                self.kernel_size, self.kernel_size)
        if supergroup != (1,2):
            raise NotImplementedError
        inv_order = [D4inverse((j,i)) for j in range(subgroupsize(supergroup, 0)) for i in range(subgroupsize(supergroup, 1))]
        for i in range(groupsize(supergroup)):
            copied_filters[:,i] = rotateflipstack_n(copied_filters[:,i], inv_order[i][1], subgroupsize(supergroup, 1), inv_order[i][0], subgroupsize(supergroup, 0))
        projected_weights = copied_filters.mean(dim=-5)
        order = [(i,j) for j in range(subgroupsize(supergroup, 0)) for i in range(subgroupsize(supergroup, 1))]
...
class DEANASNet(torch.nn.Module):
    ...
    def __init__(self, alphalr = 1e-3, weightlr = 1e-3, baseline: bool = False, superspace: tuple = (1,2), basechannels: int = 16, 
                 ...
                 reg_group: float = 0.0, name = "", parentalphas=None):
        ...
        for i in range(1,len(self.channels)):
            self.blocks.append(torch.nn.Sequential(
                MixedGroupConv2d(baseline=baseline, in_channels=self.channels[i-1], out_channels=self.channels[i], group=self.superspace, 
                                 ...
                                 skip=skip, alphas=parentalphas[i].data if parentalphas is not None else None),
        ...
        for i in range(len(self.blocks)):
            for key in self.blocks[i]._modules.keys():
                if isinstance(self.blocks[i]._modules[key], MixedGroupConv2d) or isinstance(self.blocks[i]._modules[key], MixedLiftingConv2d):
                    weightsum += self.blocks[i]._modules[key].regularization_loss(L2=L2, reg_conv=self.reg_conv, reg_group=self.reg_group)
        return weightsum
    ...
    def offspring(self, i, groupnew, verbose = False):
        ...
        #     for key in self.blocks[ind]._modules.keys():
        ...
        if verbose:
            print(weightmats)
        semi_filter = torch.stack(weightmats, dim = 1)
        ...
        if i != 0:
            semi_filter = semi_filter.reshape(-1, int(offspring.blocks[i]._modules["0"].in_channels//groupsize(groupnew)), groupsize(groupnew), semi_filter.shape[-2], semi_filter.shape[-1])
        else:
            semi_filter = semi_filter.reshape(-1, offspring.blocks[i]._modules["0"].in_channels, semi_filter.shape[-2], semi_filter.shape[-1])
        offspring.blocks[i]._modules["0"].weights[indnew] = torch.nn.Parameter(semi_filter)
        offspring.blocks[i]._modules["0"].weights[indold] = torch.nn.Parameter(torch.zeros(()), requires_grad=True)
        ...
        if groupold[1] == 2 and groupnew == (0,1):
            ...
            if groupnew == (1,0):
                if i != 0:
                    ...
                    offspring.blocks[i]._modules["0"].outchannelorders[indnew] = sum([[8*c,8*c+2,8*c+4,8*c+6,8*c+5,8*c+3,8*c+1,8*c+7] for c in range(int(outchannels/8))], start=[])
            elif groupnew == (1,1):
                if i != 0:
                    ...
                    offspring.blocks[i]._modules["0"].outchannelorders[indnew] = sum([[8*c,8*c+4,8*c+1,8*c+5,8*c+2,8*c+7,8*c+3,8*c+6] for c in range(int(outchannels/8))], start=[])
                    #offspring.blocks[i]._modules["0"].outchannelorders[indnew] = sum([[8*c,8*c+1,8*c+2,8*c+3,8*c+4,8*c+5,8*c+6,8*c+7] for c in range(int(outchannels/8))], start=[])
            elif groupnew == (0,1) and i != 0:
            ...
            if groupnew == (1,0):
                ...
                if offspring.superspace == (1,2):
                    if i != 0:
                        ...
                        offspring.blocks[i]._modules["0"].outchannelorders[indnew] = sum([[8*c,8*c+4,8*c+2,8*c+6,8*c+3,8*c+5,8*c+1,8*c+7] for c in range(int(outchannels/8))], start=[])
                else:
                    offspring.blocks[i]._modules["0"].outchannelorders[indnew] = sum([[4*c,4*c+2,4*c+1,4*c+3] for c in range(int(outchannels/4))], start=[])
            elif groupnew == (0,1):
                if offspring.superspace == (1,2):
