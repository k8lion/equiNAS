...
class MixedLiftingConv2dV2(torch.nn.Module):
    def __init__(self, group: tuple, in_channels: int, out_channels: int, kernel_size: int, padding: int = 0, bias: bool = True, baseline: bool = False, prior: bool = True, discrete: bool = False):
...
class DEANASNet(torch.nn.Module):
    ...
    def __init__(self, alphalr = 1e-3, weightlr = 1e-3, baseline: bool = False, superspace: tuple = (1,2), basechannels: int = 16, stages: int = 2, stagedepth: int = 4, pools: int = 4, kernel: int = 5, indim: int = 1, outdim: int = 10, hidden: int = 64, prior: bool = True, discrete: bool = False):
        ...
        self.blocks = torch.nn.ModuleList([])
        mlc = MixedLiftingConv2dV2(baseline=baseline, in_channels=indim, out_channels=self.channels[0], group=self.superspace, kernel_size=self.kernels[0], padding=self.kernels[0]//2, prior=prior, discrete=discrete)
        ...
        for i in range(1,len(self.channels)):
            self.blocks.append(torch.nn.Sequential(
                MixedGroupConv2dV2(baseline=baseline, in_channels=self.channels[i-1], out_channels=self.channels[i], group=self.superspace, kernel_size=self.kernels[i], padding=self.kernels[i]//2, prior=prior, discrete=discrete),
    ...
    def offspring(self, i, groupnew, verbose = False):
        ...
        if groupold[0] == 1:
            ...
            if i == 0:
                ...
                weightmats = [rotateflipstack_n(weights, r, subgroupsize(groupold, 1), f, subgroupsize(groupold, 0)) for (r,f) in order]
        else:
            if i == 0:
        ...
        inchannels = semi_filter.shape[0]*semi_filter.shape[2]
        #if i == 0:
        #    inchannels //= groupsize(self.superspace)
        if groupold[1] == 2 and groupnew == (0,1):
            ...
            offspring.blocks[i]._modules["0"].inchannelapply[indnew] = [2*c+1 for c in range(int(inchannels/2))]
            ...
            if groupnew == (1,0):
                ...
                offspring.blocks[i]._modules["0"].inchannelapply[indnew] = sum([[8*c+2,8*c+3,8*c+6,8*c+7] for c in range(int(inchannels/8))], start=[])
                offspring.blocks[i]._modules["0"].inchannelorders[indnew] = sum([[8*c+5,8*c+6,8*c+7,8*c+4,8*c+1,8*c+2,8*c+3,8*c] for c in range(int(inchannels/8))], start=[])
        elif groupold == (1,1):
            if groupnew == (1,0):
                ...
                pass
                #offspring.blocks[i]._modules["0"].outchannelorders[indnew] = sum([[8*c,8*c+4,8*c+1,8*c+5,8*c+2,8*c+7,8*c+3,8*c+6] for c in range(int(semi_filter.shape[0]*semi_filter.shape[2]/8))], start=[])
                #offspring.blocks[i]._modules["0"].inchannelapply[indnew] = sum([[8*c+7,8*c+4,8*c+5,8*c+6] for c in range(int(inchannels/8))], start=[])
                #offspring.blocks[i]._modules["0"].inchannelorders[indnew] = sum([[8*c+5,8*c+6,8*c+7,8*c+4,8*c+1,8*c+2,8*c+3,8*c] for c in range(int(inchannels/8))], start=[])
            else:
